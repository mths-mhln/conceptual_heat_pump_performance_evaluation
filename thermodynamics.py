...
# NEW: Unified PR evaluator (sub- + supercritical, no ΔT_pp_2, ṁ_ref from evaporator only)
def evaluate_cycle_PR(cycle_config, general_config, PR, verbose=True):
    ...
    supercritical_cycle = p_ref_2 > Pcrit
    ...
    # # Check isobar curvature (same as before)
    ...
    ṁ_ref = (T_h_in - T_h_pp_4) * ṁ_h * cp_h / denom_ev
    ...
    if not supercritical_cycle:
        Q_out_ref = (_specific_heat_from_isobar_path(T_ref_2, T_cond, p_ref_2, general_config, cycle_config,
                     ...
                     _specific_heat_from_isobar_path(T_cond, T_ref_3, p_ref_2, general_config, cycle_config,
                                                     supercritical_cycle=supercritical_cycle) * ṁ_ref)
        T_c_out = T_c_in + Q_out_ref / (ṁ_c * cp_c)
    else:
        Q_out_ref = _specific_heat_from_isobar_path(T_ref_3, T_ref_2, p_ref_2, general_config, cycle_config,
                                                    supercritical_cycle=supercritical_cycle) * ṁ_ref
        T_c_out = T_c_in + Q_out_ref / (ṁ_c * cp_c)

    T_h_out = T_h_in - ((_specific_heat_from_isobar_path(T_ref_1, T_ev, p_ref_1, general_config, cycle_config,
                                                         supercritical_cycle=supercritical_cycle) * ṁ_ref +
    ...
    T_c_arr = T_c_in + (np.asarray(h_ref_arr) - h_ref_3) * ṁ_ref / (ṁ_c * cp_c)
    ...
    cycle_data = dict(
        ...
        T_c_out=T_c_out, T_h_out=T_h_out,
...
def _compute_min_condenser_deltaT(cycle_data, cycle_config):
    ...
    T_c_arr = T_c_in + (h_ref_arr - h3) * m_ref / (m_c * cp_c)
...
def solve_cycle(cycle_config, general_config, verbose=True):
    ...
    def objective(PR):
        ...
        # Near-critical buffer (only relevant in supercritical region)
...
def build_ts_data(cycle_data, cycle_config, general_config):
    ...
    if T_c_pp_2 is False:
        s_c_out = s["s_ref_2"]
    elif s["Q_ref_2"] >= 1 and np.isfinite(T_c_pp_2) and not np.isclose(T_c_pp_2, T_c_in):
        s_pp_anchor = s_ref_23_v_inflection
        s_c_out = s["s_ref_3"] + (s_pp_anchor - s["s_ref_3"]) / (T_c_pp_2 - T_c_in) * (s["T_c_out"] - T_c_in)
    else:
        s_c_out = s["s_ref_2"]
