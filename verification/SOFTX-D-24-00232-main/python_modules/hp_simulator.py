...
def simulate_HP(flu,x,eta,hot,cld,n_hxc,ts_flg,*args):
    ...
    if np.isclose(props[0,1], props[0,2]) and props[3,2] > props[3,1]:
        props[:,2] = props[:,1]
       
    # size heat addition heat exchanger:
    [mdot,UAc,ppc,ppc_min] = addition_hxc(
        flu,props[:,6:10],cld[0],x[5],cld[1],cld[2],n_hxc[0:2])
...
def HP_state_points(flu,t_evap,dt_sh,pr,dt_sc,dt_ihe,etac):
    ...
    props[:,1] = compression(flu,props[:,0],1,p2,etac,satv_hp,satl_hp) 
    ...
    if dt_ihe > 0:
        ...
        h3r = h3 - (h1 - h1r)
        flu.phflash_mass(p3,h3r,0,props[:,4],t1)
        t3r = flu.fluid.T()
        s3r = flu.fluid.smass()
...
################################################################################
# run compression process: 
def compression(flu,props_in,region,p_out,eta,satv,satl):
    ...
    if (s_in > ssatv):
        flu.psflash_mass(p_out,s_in,region,satv,satv[0]+50)
    ...
    if (h_out > hsatv):
        flu.phflash_mass(p_out,h_out,region,satv,satv[0]+50)
        t_out = flu.fluid.T()
...
def addition_hxc(flu,props,tci,tco,mc,cpc,n):
    ...
    mdot = (mc*cpc*(tci - tco))/(props[2,3] - props[2,0])
    ...
    pp = np.array([pp_ev,pp_sh])    # evaporation pinch point (does not even account for fact that there can be subcooling), and superheat pinch point.
                                    # these are pp_4 and pp_1 respectively
...
def rejection_hxc(flu,props,mdot,thi,mh,cph,n): # why in the world would you push a subset of props to the fuction only to cause index confusion......
    ...
    if (p_orc < flu.pc): # then subcritical cycle:
        ...
        # so you have pc = the condensation saturation dome Q=1 point, which is of interest and commonly stringent
        # # in the general indexes, this is 2-4 
        # co condenser outlet , gneeral indexes 3-5
        # and sc is subcooling , general indexes 4-6
        # we are thus interested in the first one (likely gives inflection point 23vapour = pp2'
        # and in the last one (likely gives ref_3 point)
