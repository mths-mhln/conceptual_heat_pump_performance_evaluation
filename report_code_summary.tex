...
\textbf{Objective.} The objective of this code is to quantify \textbf{how much system COP can increase} when replacing throttling losses with a \textbf{turbine expander} and mechanical work recovery.

\subsection{Modelling Approach}

The implementation solves a steady-state vapor-compression heat pump cycle with turbine expansion, pinch-point constraints, and performance post-processing. Thermophysical properties are evaluated with \textbf{CoolProp/REFPROP}. For each candidate pressure ratio, cycle feasibility is checked and COP is evaluated.
...
\item Pinch constraints enforced via $\Delta T_{\mathrm{pp},1}$, $\Delta T_{\mathrm{pp},3}$, and $\Delta T_{\mathrm{pp},4}$.
\item Mass flow rate of refrigerant determined from the evaporator-side energy balance.
...
\textbf{Cycle evaluation for candidate $PR$}\\
...
\textbf{Objective function $f(PR)$}\\
$f(PR)=-COP_{\mathrm{turb}}$ with infeasibility penalty.
};

\node[block, right=of eval] (checks) {
\textbf{Feasibility checks}\\
Pinch constraints: $\Delta T_{\min}\geq\Delta T_{\mathrm{pp},3}$,\\
physical checks (quality/state validity),\\
near-critical buffer for robust optimization.
};

\node[block, right=of obj] (opt) {
\textbf{PR optimization}\\
Bounded search in subcritical and safe-supercritical intervals;\\
select $PR^*=\arg\max(COP_{\mathrm{turb}})$ among feasible solutions.
};
...
\textbf{Outputs}\\
Best cycle state set, selected $PR$, COP metrics, and TS/PH plotting data.
};
...
\end{tikzpicture}
\caption{\textbf{Detailed workflow of the turbine-assisted heat pump model and pressure-ratio handling.}}
\end{figure}
