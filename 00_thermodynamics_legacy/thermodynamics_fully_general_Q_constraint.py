...
#         if (PR <= 1.0 or
#             T_ref_1 > T_h_in - ΔT_pp_min_evap + 1e-3 or
#             T_ref_3 < T_c_in + ΔT_pp_min_cond - 1e-3):
#             return 1e6
...
#     bounds = [
#         (1.01, 30),                                      # PR
#         (T_h_in - 150, T_h_in - ΔT_pp_min_evap),        # T_ref_1
#         (T_c_in + ΔT_pp_min_cond, T_c_in + 150)         # T_ref_3
#     ]
...
#     T_c_pp_2 = T_c_in + (T_c_out - T_c_in) * 0.5
...
def solve_cycle(cycle_config, general_config, verbose=True):
    ...
    def objective(x):
        ...
        if (PR <= 1.0 or
            ...
            print(f"Invalid candidate: PR={PR:.4f}, T_ref_1={T_ref_1:.2f} K, T_ref_3={T_ref_3:.2f} K")
            return 1e6
        try:
            T_ev = T_ref_1 - ΔT_sh
            p_ref_1 = _cp_props("P", "T", T_ev, "Q", 0, f"REFPROP::{refrigerant}")
            ...
            w_net_per = (h_ref_2 - h_ref_1) - (h_ref_3 - h_ref_4) * cycle_config.get("ɳ_shaft", 1.0)
            if w_net_per <= 0 or q_out_per <= 0:
                print(f"Non-positive work or heat: w_net_per={w_net_per:.4f} J/kg, q_out_per={q_out_per:.4f} J/kg")
                return 1e6
            # === NEW: equality constraint on capacity =======================
            mref_req = Q_out_req / q_out_per
            if mref_req <= 0:
                print(f"Invalid mass flow requirement: mref_req={mref_req:.6f} kg/s for Q_out_req={Q_out_req:.2f} W and q_out_per={q_out_per:.4f} J/kg")
                return 1e6
            # Check pinch at *exactly* the required mass flow
            min_app_evap_val = _compute_min_approach_evap(
                mref_req, h_ref_4, h_ref_1, p_ref_1,
            ...
            if (min_app_evap_val < ΔT_pp_min_evap or
                min_app_cond_val < ΔT_pp_min_cond):
                print(f"Pinch violation at mref_req={mref_req:.6f} kg/s: min_app_evap={min_app_evap_val:.4f} K, min_app_cond={min_app_cond_val:.4f} K")
                return 1e6
            return -(q_out_per / w_net_per)   # maximise specific COP
        except Exception:
            return 1e6
    ...
    shgo_n = int(general_config.get("shgo_n", 300))
    shgo_iters = int(general_config.get("shgo_iters", 3))
    shgo_sampling = general_config.get("shgo_sampling_method", "sobol")
